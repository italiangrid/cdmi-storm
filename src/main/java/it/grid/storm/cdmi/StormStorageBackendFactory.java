...
public class StormStorageBackendFactory implements StorageBackendFactory {
   ...
   * Constructor.
   * 
   * @param configFilePath StoRM back-end plug-in configuration file path.
   * @param capabilitiesDirPath Capabilities configuration files' directory path.
   * @throws IllegalArgumentException Raised when arguments are null.
   */
  public StormStorageBackendFactory(String configFilePath, String capabilitiesDirPath)
      throws IllegalArgumentException {

    Preconditions.checkArgument(configFilePath != null, configFilePath + " path is null");
    Preconditions.checkArgument(capabilitiesDirPath != null, capabilitiesDirPath + " path is null");
    ...
    try {
      ...
      capabilities.add(mapper.readValue(new File(capabilitiesDirPath + "/container/diskonly.json"),
          StormBackendContainerCapability.class));
      capabilities.add(mapper.readValue(new File(capabilitiesDirPath + "/dataobject/diskonly.json"),
          StormBackendDatobjectCapability.class));
      capabilities
          .add(mapper.readValue(new File(capabilitiesDirPath + "/dataobject/diskandtape.json"),
              StormBackendDatobjectCapability.class));
      capabilities.add(mapper.readValue(new File(capabilitiesDirPath + "/dataobject/tapeonly.json"),
          StormBackendDatobjectCapability.class));
      exports = mapper.readValue(new File(capabilitiesDirPath + "/exports.json"),
